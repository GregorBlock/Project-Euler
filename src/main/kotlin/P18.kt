...
object P18 {
    ...
    fun main() {
        ...
        println("Maximum path sum I ${maximumPathSumI(triangle)}")
    }

    fun maximumPathSumI(triangle: Array<IntArray>, y: Int = 0, x: Int = 0): Int {
        var value = triangle[y][x]
        if (triangle.size - 1 > y && triangle[y].size -1 > x) {
            val left = maximumPathSumI(triangle, y + 1, x)
            val right = maximumPathSumI(triangle, y + 1, x + 1)
            value += if (left > right) {
                left
            } else {
                right
