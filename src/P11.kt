...
fun main() {
    ...
    println("Largest product in a grid ${greatestProduct(matrix)}")
}

fun greatestProduct(matrix: Array<IntArray>): Long {
    var greatestProduct = Long.MIN_VALUE

    // diagonally right
    for (y in 0..19) {
        for (x in 0..19) {
            if (x + 3 < 20 && y + 3 < 20) {
                val temp = (matrix[x][y] * matrix[x + 1][y + 1] * matrix[x + 2][y + 2] * matrix[x + 3][y + 3]).toLong()
    ...
    for (y in 0..19) {
        for (x in 0..19) {
            if (x + 3 < 20 && y + 3 < 20) {
                val temp = (matrix[x][y] * matrix[x][y + 1] * matrix[x][y + 2] * matrix[x][y + 3]).toLong()
                if (temp > greatestProduct) {
                    greatestProduct = temp
                }
            }
    ...
    for (y in 0..19) {
        for (x in 0..19) {
            if (x - 3 >= 0) {
                val temp = (matrix[x][y] * matrix[x - 1][y] * matrix[x - 2][y] * matrix[x - 3][y]).toLong()
                if (temp > greatestProduct) {
                    greatestProduct = temp
                }
            }
